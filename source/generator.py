...
def makeModulusMeme(file: str, startingNum: int, endingNum: int, beginningText: str, sentenceArray: List[str], endingText: str):
	...
	with open(file, "a") as openFile:
		...
		if startingNum != endingNum:
			#print("start: " + str(startingNum) + ", end:" + str(endingNum))
			for i in range(startingNum, endingNum):
				line = sentenceArray[i % len(sentenceArray)]
				fileContentToAdd += re.sub(r'\*', str(i), line)
...
if numOfCommands == 2:
	...
	makeModulusMeme('meme.py', 2, 1000, 'def isEvenOrOdd(num: int):\n\tif num == 1:\n\t\tprint("odd")', ['\telif num == *:\n\t\tprint("even")\n','\telif num == *:\n\t\tprint("odd")\n'], 
				'\telse:\n\t\tprint("God there has to be an easier way.")')